...
export class RestanService {
  ...
  async calculateRestanWarnings() {
    ...
    for (const h of pendingHarvests) {
      const harvestTimestamp = Number(h.clientTimestampMs) || h.createdAt.getTime();
      const elapsedHours = (now - harvestTimestamp) / (1000 * 60 * 60);

      let stage = 'NORMAL';
      let estimatedFfa = 1.20; // Standar TBS segar

      if (elapsedHours >= 24) {
        stage = 'RESTAN_OVERDUE';
        // Formula FFA: 1.50% + 0.15% per jam setelah 24 jam
        estimatedFfa = Number((1.50 + 0.15 * (elapsedHours - 24)).toFixed(2));
    ...
    return {
      ...
      criticalRestanCount: warnings.filter((w) => w.stage === 'RESTAN_OVERDUE').length,
      warningList: warnings,
    };
  }

  async markAsPickedUp(harvestId: string) {
    const harvest = await this.harvestRepo.findOne({ where: { id: harvestId } });
    if (!harvest) {
