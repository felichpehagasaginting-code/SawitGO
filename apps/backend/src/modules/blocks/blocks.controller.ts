...
export class BlocksController {
  ...
  @Get('blocks')
  @ApiOperation({
    summary: 'Mengambil master poligon blok untuk cache offline',
  })
  async getBlocks() {
  ...
  @ApiOperation({
    summary: 'Validasi Spasial Point-in-Polygon EUDR (ST_Contains)',
  })
  async verifyPoint(
    ...
    return this.blocksService.verifyPointInBlock(
      blockId,
      Number(lat),
      Number(lng),
    );
