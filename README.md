<div align="center">
  ...
  <p align="center">
    <img src="assets/stickers/sticker_offline_ready.svg" width="120" height="120" alt="Offline Ready" />
    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
    <img src="assets/stickers/sticker_eudr_traceable.svg" width="120" height="120" alt="EUDR Traceable" />
  </p>
...
Di sebagian besar perkebunan kelapa sawit Indonesia, **kendala sinyal seluler (*blankspot*)** di areal blok perkebunan menyebabkan keterlambatan rekapitulasi data panen. Hal ini berdampak langsung pada penumpukan buah di TPH (**Restan $> 24$ jam**), yang memicu lonjakan kadar **Asam Lemak Bebas (FFA $> 5\%$)** dan mendegradasi harga jual TBS serta rendemen CPO. Di sisi lain, regulasi internasional seperti **EU Deforestation Regulation (EUDR No 2023/1115)** mewajibkan bukti poligon geospasial persil lahan berakurasi tinggi ($< 5.0$ meter).

**SawitGO (AgriSync)** hadir sebagai solusi *cyber-physical* komprehensif yang menggabungkan aplikasi mobile *offline-first* dengan enkripsi hardware, mesin resolusi konflik berbasis bobot jabatan di backend, dan dashboard visual eksekutif bergaya *Industrial Command Center*.
...
graph TB
    ...
    subgraph SECURITY_TIER ["🛡️ Network & Gateway Tier"]
        TLS_GATEWAY["TLS 1.3 Pinning / HTTPS REST Gateway"]
        AUTH_JWT["JWT Bearer Authentication\n+ Device Idempotency Guard"]
    end

    subgraph BACKEND_TIER ["⚙️ Cloud & Analytics Tier (NestJS Engine)"]
    ...
    subgraph WEB_TIER ["💻 Executive Web Tier (Next.js Dashboard)"]
        COMMAND_CENTER["Executive Command Center\n(Lenis + GSAP Odometer + Bento Grid)"]
    end
...
* **Operasional 100% Offline**: Pencatatan janjang, brondolan, dan grading mutu buah (mentah, masak, lewat masak, tangkai panjang) tersimpan instan ke **Isar DB** tanpa lag.
...
* **Background Worker Cerdas**: Listener konektivitas memeriksa status jaringan setiap 30 detik (`connectivity_plus`). Begitu perangkat mendeteksi sinyal (Wi-Fi/4G), sistem langsung mengirimkan batch payload secara otomatis.

### 2. ⚖️ Domain-Specific Conflict Resolution (Weighted RBAC)
* Menyelesaikan masalah tabrakan data (*data collision*) saat banyak aktor mengedit TPH yang sama dalam kondisi offline:
  $$\mathbf{Priority\ Score} = (\text{Role Weight} \times 1.000.000.000.000) + \text{Timestamp (ms)}$$
* **Hierarki 5 Jenjang Perkebunan**:
  - `Manager (Weight 5)`: Otoritas tertinggi lintas estate.
  - `Askep (Weight 4)`: Otorisasi & koreksi lintas afdeling.
  - `Asisten (Weight 3)`: Verifikasi & revisi data mandor di afdeling.
  - `Mandor (Weight 2)`: Pengawasan mutu kemandoran panen.
  - `Krani (Weight 1)`: Input awal pencatatan di TPH.
* **Audit Trail**: Seluruh tindakan konsensus (`ACCEPTED_NEW`, `ACCEPTED_OVERWRITE`, `IGNORED_IDEMPOTENT`, `REJECTED_STALE`) tercatat lengkap di tabel `sync_audit_trails`.

### 3. 📡 Realtime Offline P2P Mesh Synchronization
* Pertukaran data antar-perangkat di tengah kebun menggunakan konektivitas lokal **Wi-Fi Direct / Bluetooth Low Energy (BLE)** tanpa kuota internet.
* Mengadopsi pola **Data Mule**: Truk angkut TBS dan sepeda motor mandor patroli bertindak sebagai kurir data nirkabel yang membawa data dari TPH terisolasi ke Pos Timbang/Pabrik (PKS).
...
* **Ekspor Kepatuhan**: Mendukung ekspor data ketertelusuran persil ke format resmi **GeoJSON FeatureCollection**.
...
* Dashboard interaktif berbasis **Next.js 14 App Router** dengan palet warna *Deep Forest & Emerald Neon*.
* Pengalaman visual kelas enterprise yang memadukan **Lenis Smooth Scroll**, **GSAP Odometer Counting**, dan **Framer Motion Bento Grid**.
...
│       ├── src/providers/       # Lenis + GSAP Smooth Scroll Provider
│       └── src/components/      # Interactive GIS Map & Bento Metric Cards
├── assets/                      # 100% Vector SVG Brandings & Stickers
├── docs/                        # 11 Dokumen Spesifikasi Formal (SSOT)
...
cd apps/backend
npm install
# Pastikan konfigurasi .env sesuai (host, port 5432, user, password, db: sawitgo_db)
npm run start:dev
...
### 3. Menjalankan Executive Web Dashboard (Next.js)
```bash
cd apps/web
npm install
npm run dev
...
| 2 | 🎨 **[ui_ux_frontend.md](docs/ui_ux_frontend.md)** | Panduan Desain Anti-AI Slop: GSAP, Lenis, Framer Motion, Ergonomi Lapangan & 100% SVG |
| 3 | 🏛️ **[system_architecture.md](docs/system_architecture.md)** | System Architecture Document (SAD), 4-Tier Topology, Clean Architecture, Enkripsi AES-256 |
| 4 | 🗄️ **[database_schema.md](docs/database_schema.md)** | DDL PostgreSQL + PostGIS (Polygon/Point SRID 4326), ERD, dan Isar DB Models |
| 5 | ⚖️ **[conflict_resolution.md](docs/conflict_resolution.md)** | Algoritma Resolusi Konflik, Formula Priority Score & Matriks RBAC 5 Jenjang |
| 6 | 🔌 **[api_contract.md](docs/api_contract.md)** | Spesifikasi OpenAPI 3.1, Endpoint `/api/v1/sync/batch`, Envelope Schema & Error Codes |
| 7 | 📊 **[flowcharts_and_dfd.md](docs/flowcharts_and_dfd.md)** | DFD Level 0/1, Flowchart Local Sync, Conflict Engine, dan State Machine Restan |
| 8 | 🗺️ **[geospatial_traceability.md](docs/geospatial_traceability.md)** | Standar GeoJSON WGS84, Algoritma Point-in-Polygon & Validasi Audit EUDR/RSPO/ISPO |
| 9 | 🌾 **[business_domain_logic.md](docs/business_domain_logic.md)** | Logika Bisnis Agronomi: BJR, Model Degradasi FFA, Toleransi Mutu Buah |
| 10 | 📦 **[monorepo_and_git_conventions.md](docs/monorepo_and_git_conventions.md)** | Tata Kelola Monorepo, Aturan Branching Git Flow, dan Conventional Commits |
| 11 | 🧪 **[test_matrix_and_sus.md](docs/test_matrix_and_sus.md)** | Matriks Uji Lab Blankspot (Latensi, Throttling) & Kuesioner SUS 10 Butir |
